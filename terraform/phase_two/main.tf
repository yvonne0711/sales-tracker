...
resource "aws_lambda_function" "c19-sales-tracker-lambda-next" {
  ...
  role          = aws_iam_role.c19-sales-tracker-lambda-execution-role.arn
  package_type  = "Image"
  image_uri     = "129033205317.dkr.ecr.eu-west-2.amazonaws.com/c19-sales-tracker-ecr-next:latest"
  memory_size   = 512
  timeout       = 30
  architectures = ["x86_64"]
...
resource "aws_iam_role_policy" "c19-sales-tracker-scheduler-trigger-sfn" {
  role = aws_iam_role.c19-sales-tracker-scheduler-role.id
  policy = jsonencode(
    {
      Version = "2012-10-17",
      Statement = [
        {
          ...
          Resource = "arn:aws:states:eu-west-2:129033205317:stateMachine:c19-sales-tracker-state-machine"
        },
      ]
    }
  )
...
resource "aws_scheduler_schedule" "c19-sales-tracker-scheduler" {
  ...
  target {
    arn      = "arn:aws:states:eu-west-2:129033205317:stateMachine:c19-sales-tracker-state-machine"
  ...
  container_definitions = jsonencode([
    {
      ...
      image     = "129033205317.dkr.ecr.eu-west-2.amazonaws.com/c19-sales-tracker-ecr-dashboard",
      cpu       = 10,
      memory    = 512,
      essential = true
      portMappings = [
