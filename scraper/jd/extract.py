...
def get_name(url: str, name_class: str, headers: dict[str:str]) -> str:
...
def get_current_price(url: str, cost_class: str, discounted_class: str, name_class, headers: dict[str:str]) -> str:
    """Returns the current price of a product from its details."""
    name_product = get_name(url, name_class, headers)
    if is_discounted(url, discounted_class, headers):
        return name_product, scrape_price(url, discounted_class, headers)
    return name_product, scrape_price(url, cost_class, headers)
